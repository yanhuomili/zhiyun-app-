<template>
  <div class="quoteTrailerDetail" id="quoteTrailerDetail">
    <div class="quote-info">
      <div class="quote-detail">
        <dl class="clearfix quote-detail-dl">
          <dt class="fl">装船码头：</dt>
          <dd class="fl">{{quoteData.trailerStartPort}}</dd>
        </dl>
        <dl class="clearfix quote-detail-dl">
          <dt class="fl">装货区域：</dt>
          <dd class="fl">{{quoteData.trailerDistrict}}</dd>
        </dl>
        <dl class="clearfix quote-detail-dl">
          <dt class="fl">装货地点：</dt>
          <dd class="fl">{{quoteData.trailerLoadingPlace}}</dd>
        </dl>
      </div>
    </div>
        ...
        <tbody>
          <tr class="tr" v-if="quoteData.quoteContainerTypeVOList">
            ...
            <td class="td-price-all">
              <div class="price-all-top">拖车费(OceanFreight)</div>
    ...
    <footer-remark v-model="quoteData.trailerRemark"></footer-remark>
  </div>
</template>
<script>
  ...
  export default {
    data() {
      return {
        ...
        isRemarkShow: false,
        quoteData: { trailerQuoteTmpVOList: [] },
      ...
      async getQuoteShow() {
        try {
          ...
          if (data.returnCode === 0) {
            ...
            // this.surchargeRouteServiceVOList = this.covertSurcharge(data.data.trailerQuoteTmpVOList)
          }
          console.log(data)
        } catch (error) {
          throw new Error()
